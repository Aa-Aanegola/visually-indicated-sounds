...
# from AudioDataPoint import AudioDataPoint
...
def createDatapointsFromFile(file_name, window_duration=0.5):
    ...
    for row in annotations.iterrows():
        ...
        data_points.append(window_sound)
...
def processFile(file_name, idx, total_files):
    ...
    try:
        ...
        for data_point in data_points:
            ...
            readLock.release()
            with open(f'/scratch/aanegola/audio_data/train/{n_points}.pkl', 'wb') as f:
                pickle.dump(data_point.astype(np.float32), f)
    except:
        pass
