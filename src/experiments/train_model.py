...
trainDataLoader = DataLoader(trainDataset, batch_size=BATCH_SIZE,
                             collate_fn=visCollate, shuffle=True, num_workers=6)
valDataLoader = DataLoader(
    valDataset, batch_size=BATCH_SIZE, collate_fn=visCollate, num_workers=6)
...
class VISTrivialModel(pl.LightningModule):
    ...
    def forward(self, stFrames, frame0):
        ...
        frame0Features = self.featureExtractor(
            frame0).unsqueeze(1).repeat(1, stFrames.shape[1], 1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # loss = nn.MSELoss()(out, coch) #log_2
...
model = VISTrivialModel.load_from_checkpoint("tb_logs/trivial_model/version_7/checkpoints/epoch=24-step=31450.ckpt", outputSize=42)
